...
const noLink = checker.linkage(
  { hash: 'A'.repeat(64) },
  { view: { txHashDisplay: 'BBBBBBBB…CCCC' } },
  [{ hash: 'A'.repeat(64) }],
);
assert.equal(noLink.strongJoinKeyFound, false);
assert.equal(noLink.directFullHashMatch, false);
assert.equal(noLink.prefixMatches, 0);
assert.equal(noLink.status, 'No Strong Link Between Proof And XRPL Tx');

for (const marker of ['XRPL anchor registry', 'Proof-side record', 'Verifier endpoint', 'Linkage']) {
  assert.equal(page.includes(marker), true, `Missing panel: ${marker}`);
}
assert.equal(page.includes('This tool does not prove that a proof and XRPL transaction are the same record.'), true);
assert.equal(runtime.includes('/api/proof-anchor?input='), true);
assert.equal(runtime.includes("link.strongJoinKeyFound ? 'Candidate string' : 'Unresolved'"), true);
assert.equal(endpoint.includes('checker.check(input)'), true);
assert.equal(core.includes("'No Strong Link Between Proof And XRPL Tx'"), true);
